...
**Not done yet (any version):** no 404 handling when updating/deleting an unknown id (`deleteById` on a missing id throws rather than no-ops now — a real behavior change from v1 — but proper `@ExceptionHandler`/`ResponseStatusException` handling is deliberately deferred, it's API-design work not storage-layer work); no edit/update UI on the frontend; no seed data.
