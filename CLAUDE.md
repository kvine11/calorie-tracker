...
## Current frontend structure (as of the Oxide revamp, 2026-09-07)

The v1 component list in "Current state" is historical. What actually renders today:

- **`App.jsx`** owns `view` state (`"today" | "history" | "search" | "settings"`) — hand-rolled navigation, **not** `react-router-dom`, which is still not a dependency. It also owns `entryDate`, the meal list, and the confirm/undo dialogs.
- **Shell:** `Sidebar.jsx` (238px, sticky) + one of four view panes. `QuickAddPalette.jsx` is the Cmd/Ctrl-K fast-add path. `ConfirmDialog.jsx` and `UndoToast.jsx` sit above everything.
- **Views:** `TodayView.jsx` (the two-column `[data-grid]` — ring left, form + list right), `HistoryView.jsx`, `SearchView.jsx`, `SettingsView.jsx`.
- **Pieces:** `CalorieRing.jsx`, `MealList.jsx`, `FoodItem.jsx`, `MealForm.jsx`, `DateSelection.jsx`, `MacroLine.jsx`, `Icons.jsx`.
- **Deleted in the revamp (`1be4438`):** `Header.jsx`, `CalorieSummary.jsx`. Do not go looking for them.
- **Shared modules:** `api.js` (`getMealsByDate`/`addMeal`/`updateMeal`/`deleteMeal`/`searchMeals`), `hooks.js` (`useDebounce`, `useFoodSearch`, `useClickOutside`, `useCountUp`), `dates.js` (`iso`, `parseISO`, `todayISO`, `shiftISO`, `weekOf`, `longDate`, `shortDate`), `macros.js`.
...
- A 238px sticky `Sidebar` that shrinks to 190px at ≤900px (`index.css`) but **never collapses** — on a ~390px phone viewport it takes half the screen. This is the actual break, and the first thing to fix whenever mobile is picked up.
