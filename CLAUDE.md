...
- **v4.5 — Search UX: bottom sheet (next up, new, not in the original Notion roadmap):** the v4 PR's search feature (see "Current state") used an inline dropdown under the Food field that turned out to visually cover the Cal field, Save/Cancel buttons, and the date strip once results filled it out — a real usability problem, not just the z-index bug that was fixed first. See "Next steps" below for the design.
...
- Backend: `FoodSearchService` calls FatSecret's OAuth2 client-credentials token endpoint (caching the token until `expires_in` elapses), then `GET /rest/foods/search/v1`, parsing calories out of `food_description` via regex — FatSecret's search endpoint doesn't return structured macros, which is a real limitation to revisit if v6's original "pull full protein/carbs/fat" goal ever gets picked back up. `FoodSearchController` exposes this at `GET /api/meals/search?query=...`, grouped under `/api/meals` alongside `MealEntryController`'s endpoints rather than its own top-level path.
...
- **Known limitation, not fixed here — see "Next steps: v4.5" below:** the inline dropdown covers the Cal field, Save/Cancel, and the date strip once it has enough results. The z-index/close-on-outside-click bugs above are fixed, but the underlying "a tall list squeezed into a narrow card" layout problem isn't — that's what v4.5 replaces.
...
## Next steps: v4.5 — Search UX (bottom sheet)

Not started. Replaces the inline suggestions dropdown built under v4 (see "Current state") — it visually covers the Cal field, Save/Cancel buttons, and the embedded date strip once results fill it out, which is a real usability problem, and especially so on mobile (which this app is meant to support eventually, per explicit user direction — there's no side-panel space to lean on there, ruling out a side-anchored dropdown as an alternative fix).

Design decided in conversation before any code was written, worth sticking to rather than re-deriving:

- **One new shared component**, `FoodSearchSheet.jsx` (not yet created), used by both `MealForm` and `FoodItem` — consolidates the `useDebounce`/search-effect/`suggestions` logic currently duplicated across both files, and now also owns its own name + calories inputs and a confirm action (rather than just a list of suggestions).
- **Opens on focus/click** of the Food field (MyFitnessPal/CalAI-style: the search experience opens immediately, then the user types), not only once results exist like the current dropdown.
- **Calories stays inside the sheet**, not just the name/search box — the "type a name, get no matches, type calories manually" flow (the original feature's core requirement) needs to keep working entirely within the sheet, without falling back to a separate on-page Cal field.
- **The sheet never calls `onAdd`/`onUpdate` directly** — it only reports `{ name, calories }` back up via an `onSubmit` prop and closes itself, staying reusable rather than assuming what its caller wants to do. What each caller does with that result differs, and is a real fork to keep straight while building both integration points:
  - `MealForm`: `onSubmit` calls `onAdd(name, calories)` immediately and closes — matches today's one-step "Add."
  - `FoodItem`: `onSubmit` only sets `draftName`/`draftCalories` and closes — the existing Save button still does the real `onUpdate` call, since it also needs to bundle in `draftDate`, which the sheet has no concept of.
- **Explicitly deferred, not part of v4.5:** a real dedicated search *page*/route (the actual MyFitnessPal pattern) — would need client-side routing (e.g. `react-router-dom`, not currently a dependency) or at least a hand-rolled top-level "view" state in `App.jsx`, which is a navigation-model decision for the whole app, not just this one feature. Revisit as its own version once/if that gets decided deliberately.
